...
const MailTemplatesView: React.FC<MailTemplatesViewProps> = ({ currentUserRole }) => {
    ...
    const TemplateCard: React.FC<{ template: MailTemplate }> = ({ template }) => (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-md p-6 hover:shadow-lg transition-all group flex flex-col h-full">
            <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                    {!template.isDraft && (
                        <button onClick={() => openPreview(template)} className="w-8 h-8 flex items-center justify-center rounded-md hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-indigo-600 transition-colors" title="Preview">
                    ...
                    {canEdit && (
                        <>
                            {template.isDraft && (
                                <button onClick={() => handlePublishDraft(template)} className="w-8 h-8 flex items-center justify-center rounded-md hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-emerald-600 transition-colors" title="Publish">
                                    <i className="fa-solid fa-upload text-sm"></i>
                                </button>
                            )}
                            <button onClick={() => openModal(template)} className="w-8 h-8 flex items-center justify-center rounded-md hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-blue-600 transition-colors" title="Edit">
                                <i className="fa-solid fa-pen text-sm"></i>
                            </button>
                            <button onClick={() => handleDelete(template.id)} className="w-8 h-8 flex items-center justify-center rounded-md hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-rose-600 transition-colors" title="Delete">
    ...
    const EmptyState = ({ type }: { type: Tab }) => (
        <div className="bg-white dark:bg-slate-900 rounded-md p-12 text-center border border-slate-200 dark:border-slate-800">
            ...
            {canEdit && (
                <button onClick={() => openModal()} className="bg-indigo-600 text-white px-5 py-2.5 rounded-md text-sm font-bold shadow-lg hover:bg-indigo-700 transition-all active:scale-95">
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <header className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                ...
                {canEdit && (
                    <button
                        onClick={() => openModal()}
                        className="bg-indigo-600 text-white px-5 py-2.5 rounded-md text-sm font-bold shadow-lg transition-all active:scale-95 hover:bg-indigo-700 self-start sm:self-auto"
                    >
            ...
            <div className="flex gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-md w-fit">
                {(['published', 'drafts'] as Tab[]).map(tab => (
                    <button
                        ...
                        className={`px-5 py-2 rounded-md text-sm font-bold capitalize transition-all ${activeTab === tab
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-2xl bg-white dark:bg-slate-900 rounded-md shadow-2xl overflow-hidden z-50 animate-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
                            ...
                            <div>
                                <div className="mb-2 text-xs text-slate-400 leading-relaxed">
                                    ...
                                    {Object.keys(SAMPLE_REQUEST).map(k => (
                                        <code key={k} className="bg-slate-100 dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 px-1 py-0.5 rounded-md text-xs mr-1 mb-1 inline-block">{`{{${k}}}`}</code>
                                    ))}
                                </div>
                                <TextArea
                        ...
                        <div className="p-6 border-t dark:border-slate-800 bg-slate-50 dark:bg-slate-800/30 flex justify-between items-center gap-3">
                            <div>
                                {!currentTemplate.id && (currentTemplate.name || currentTemplate.subject || currentTemplate.body) && (
                                    <button
                                        ...
                                        className="px-4 py-2 text-rose-500 font-semibold hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-md transition-all text-sm"
                                    >
                            ...
                            <div className="flex gap-3">
                                <button
                                    ...
                                    className="px-5 py-2.5 text-slate-500 font-bold hover:bg-slate-200 dark:hover:bg-slate-700 rounded-md transition-all text-sm"
                                >
                                    Cancel
                                </button>
                                {canEdit && (
                                    <>
                                        <button
                                            ...
                                            disabled={saving}
                                            className="px-5 py-2.5 bg-amber-500 text-white font-bold rounded-md hover:bg-amber-600 transition-all text-sm disabled:opacity-60 shadow"
                                        >
                                        ...
                                        <button
                                            ...
                                            disabled={saving}
                                            className="px-5 py-2.5 bg-indigo-600 text-white font-bold rounded-md shadow-lg hover:bg-indigo-700 transition-all text-sm disabled:opacity-60"
                                        >
            ...
            {isPreviewOpen && previewTemplate && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-3xl bg-white dark:bg-slate-900 rounded-md shadow-2xl overflow-hidden z-50 animate-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
                        <div className="px-8 py-5 bg-indigo-600 text-white flex justify-between items-center">
                            ...
                            <button onClick={() => setIsPreviewOpen(false)} className="bg-white/10 hover:bg-white/20 p-2 rounded-md transition-colors">
                                <i className="fa-solid fa-xmark"></i>
                            </button>
                        </div>
                        <div className="p-8 overflow-y-auto custom-scrollbar flex-1 bg-slate-50 dark:bg-slate-950">
                            <div className="bg-white rounded-md shadow-sm border border-slate-200 mx-auto max-w-2xl overflow-hidden">
                                <div
                                    className="p-8"

-- ============================================================
-- Mail Templates: Full Setup (Run this in Supabase SQL Editor)
-- ============================================================

-- 1. Create the table (with is_draft column included from the start)
...
-- 2. Partial unique index: only published (non-draft) templates need a unique status_trigger
...
-- 5. RLS: Only Admin can insert, update, delete
...
-- 6. Insert default published templates
INSERT INTO public.mail_templates (name, subject, status_trigger, body, is_draft) VALUES
(
...
(
  ...
  '<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="background-color: #FFF9C4; border-left: 4px solid #FBC02D; padding: 15px; margin: 20px 0;">
      <p style="margin: 0; color: #F57F17; font-weight: bold; font-size: 14px;">⚠ Travel Policy Violated</p>
      <p style="margin: 5px 0 0 0; color: #555; font-size: 13px;">Your request does not meet the advance booking policy requirements.</p>
    </div>
    <div style="margin: 20px 0;">
      <p style="margin: 5px 0; color: #333; font-size: 14px;"><strong>Purpose:</strong> {{purpose}}</p>
      <p style="margin: 5px 0; color: #333; font-size: 14px;"><strong>From:</strong> {{from}}</p>
    ...
    <p style="color: #333; font-size: 14px; line-height: 1.6;"><strong>Action Required:</strong> Please provide a reason for this urgent travel request. Your request will be sent for approval once the reason is submitted.</p>
